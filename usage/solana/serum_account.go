...
func decodeOrderBookAccount(account *rpc.Account) *OrderBookLayout {
	accountData := account.Data.GetBinary()
	orderBook := OrderBookLayout{}
	err := unpackOrderBookLayout(accountData, &orderBook)
	if err != nil {
		panic(err)
	}
	return &orderBook
...
func bids(bid *OrderBookLayout, size int) []*Tick {
	ticks := make([]*Tick, 0)
	bidItems := bid.Slab.items(true, 1)
...
func asks(ask *OrderBookLayout, size int) []*Tick {
	ticks := make([]*Tick, 0)
	askItems := ask.Slab.items(false, 1)
